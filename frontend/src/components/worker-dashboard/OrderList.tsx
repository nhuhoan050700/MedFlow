...
export default function OrderList() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      setError('Cannot reach server. Set DATABASE_URL (Railway Postgres) in Vercel env if you see no orders.')
      setItems([])
    } finally {
      setLoading(false)
    }
  ...
  const getStatusLabel = (status: string) => {
    if (['pending', 'paid', 'assigned'].includes(status)) return 'Pending'
  ...
  if (error) {
    return (
      <div className="bg-white rounded-lg shadow-lg p-8 text-center">
        ...
        <p className="mt-2 text-sm text-gray-500">Set DATABASE_URL in Vercel (Railway Postgres). Orders are loaded from the database.</p>
      </div>
    )
  }
